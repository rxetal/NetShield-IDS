...
# دالة لقراءة النتايج ديناميكياً من ملفات التقرير بدون تكرار النسبة المئوية
def parse_accuracy_from_file(file_path, patterns):
        ...
        with open(file_path, "r") as f:
            ...
            for pattern in patterns:
                ...
                if match:
                    ...
                    # إذا كانت القيمة تم تحويلها مسبقاً لمئوية (مثال: 99.22)، لا تضرب في 100 مرة أخرى
                    if acc_val > 1.0:
                        return f"{acc_val:.2f}%"
                    return f"{acc_val * 100:.2f}%"
    return "N/A"

def load_real_metrics():
    t1_acc = parse_accuracy_from_file(
        "results/metrics/tier1_report.txt", 
        [r"Acc:\s*([\d\.]+)", r"Accuracy:\s*([\d\.]+)"]
    )
    t2_acc = parse_accuracy_from_file(
        "results/metrics/tier2_report.txt", 
        [r"Accuracy:\s*([\d\.]+)", r"Acc:\s*([\d\.]+)"]
    )
    e2e_acc = parse_accuracy_from_file(
        "results/metrics/end_to_end_report.txt", 
        [r"End-to-End Pipeline Overall Accuracy:\s*([\d\.]+)", r"Accuracy:\s*([\d\.]+)"]
    )
    return t1_acc, t2_acc, e2e_acc
...
if menu == "📊 System Dashboard & Metrics":
    ...
    # قراءة المقاييس الحقيقية
    t1_acc, t2_acc, e2e_acc = load_real_metrics()

    col1, col2, col3 = st.columns(3)
    col1.metric("Tier 1 Accuracy", t1_acc, "Normal vs Attack")
    col2.metric("Tier 2 Accuracy", t2_acc, "Multiclass Attacks")
    col3.metric("System End-to-End Accuracy", e2e_acc, "Full Pipeline")
    ...
    with c1:
        ...
        if os.path.exists(t1_cm_path):
            ...
            st.info("Run training to generate Tier 1 plot.")
    ...
    with c2:
        ...
        if os.path.exists(t2_cm_path):
            ...
            st.info("Run training to generate Tier 2 plot.")
    ...
    with d2:
        st.write("**Tier 2: Attack Categories Breakdown**")
...
# ==========================================
    ...
    with tab1:
        ...
        ms_chart_1 = "results/figures/model_selection_comparison.png"
        ms_chart_2 = "results/figures/model_selection_f1.png"

        if os.path.exists(ms_chart_1):
            st.image(ms_chart_1, use_container_width=True)
            st.success("✅ XGBoost was selected as the core architecture due to superior F1-score performance.")
        elif os.path.exists(ms_chart_2):
            st.image(ms_chart_2, use_container_width=True)
            st.success("✅ XGBoost was selected as the core architecture due to superior F1-score performance.")
        else:
    ...
    with tab2:
        ...
        if os.path.exists(unsup_chart):
            ...
            st.caption("Visualizing high-dimensional UNSW-NB15 flow data projected onto principal components.")
        else:          
